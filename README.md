...
```
docker run --rm --entrypoint="/bin/sh" krdan/tsdns:3.12.0 -c "cat tsdns_settings.ini.sample" > tsdns_settings.ini
```
...
```
docker run -d -p 41144:41144 -v $(pwd)/tsdns_settings.ini:/var/tsdns/tsdns_settings.ini:ro --name tsdns krdan/tsdns:3.12.0
```
